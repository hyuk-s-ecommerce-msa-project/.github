...
- 카프카 order-cancel-topic은 현재 userService.addPoint를 단순 호출하고 있음
  - 만약 네트워크 순단이 발생해 같은 메시지가 두 번 들어올때 중복으로 복구될 위험이 있다
  - 프로듀서에서 카프카에 메시지를 보냈는데 네트워크 순단이 발생해 정상적으로 ACK 응답을 보내지 못하면
  - 프로듀서는 메시지 전송에 실패했다 판단하고 retries 옵션에 따라 한 번 더 메시지를 보내게 됨
  - 그럼 네트워크가 복구되고 두 번의 메시지가 날라가게 되어 포인트가 중복으로 복구될 수 있음
  - 이를 해결하려면, point history라는 멱등성 테이블을 user안에 만들어두고, 주문 시 포인트 차감 API를 호출할 때 orderId와 함께 포인트를 추적할 수 있도록 point history 테이블에 안에 저장하고, order-cancel-topic이 호출될 때 검증하는 로직을 추가해주면 방지할 수 있을 것으로 보임

...
- 카프카 order-cancel-topic은 현재 userService.addPoint를 단순 호출하고 있음
 ...
 2026.3.19
  - 깃허브 관리의 필요성
    - commit 내역을 보면 대량으로 코드를 커밋하고 있는데, 이는 코드를 추적하고 관리하는데 어려움이 있음
    - 현재 보이는 문제점으로는
      - 하나의 로직 단위로 커밋을 한게 아닌 여러 개의 로직과 심지어, 다른 MSA 서비스들의 로직도 합쳐서 보낸 커밋이 몇 개 있음
        -  이렇게 되면, 로직의 흐름을 파악하기 어려워 버그를 놓치게 될 수도 있음
        -  또한, 메인 브랜치에 merge 할때 버그가 존재하는 채로 merge하게 되는 문제도 발생 가능
      - 브랜치를 분리하여 관리하지 않았음
        - 현재는 하나의 브랜치에 모든걸 다 커밋했는데, 이럴 경우 메인 브랜치에 불완전한 코드가 섞이거나,
        - 여러 명의 개발자가 동시에 작업할 경우, 서로의 코드가 섞여 pull과 push 과정에서 충돌이 발생할 수도 있음
        - 또한, 기능 단위의 실험이 불가능함. 특정 기능을 시도해 보다가 포기하고 싶을 때, 단일 브랜치에서는 그 사이사이에 섞인 다른 정상적인 커밋들 때문에 해당 기능만 제거하기가 힘듬
    - 결론
      - 개인 프로젝트라서가 아니라, 협업 하는 환경이라 생각하고 커밋과 브랜치 관리에 신경을 썼어야 함
